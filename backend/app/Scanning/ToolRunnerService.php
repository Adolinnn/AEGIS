...
class ToolRunnerService
{
    ...
    /**
     * Run multiple tools concurrently in a process pool.
     *
     * @param  array<string, array{tool: ToolName, command: array<int, string>, timeout?: int|null, idleTimeout?: int|null, outputCapKb?: int}>  $tasks
     * @return array<string, ToolRunResult>
     */
    public function runConcurrent(array $tasks, int $defaultOutputCapKb = 4096): array
    {
        if (empty($tasks)) {
            return [];
        }
        ...
        try {
            $poolResults = Process::concurrently(function (\Illuminate\Process\Pool $pool) use ($tasks) {
                foreach ($tasks as $key => $task) {
                    $tool = $task['tool'];
                    $timeout = $task['timeout'] ?? $tool->timeout();
                    $idleTimeout = $task['idleTimeout'] ?? $tool->idleTimeout();
            ...
            $results = [];
            foreach ($tasks as $key => $task) {
                $process = $poolResults[(string) $key] ?? null;
                $outputCapKb = $task['outputCapKb'] ?? $defaultOutputCapKb;

                if (! $process) {
                    $results[$key] = new ToolRunResult(
                        raw: '',
                        exitCode: -1,
                        timedOut: false,
            ...
            Log::warning('Concurrent tool execution timed out', ['error' => $e->getMessage()]);
            ...
            Log::error('Concurrent tool execution failed', ['error' => $e->getMessage()]);
